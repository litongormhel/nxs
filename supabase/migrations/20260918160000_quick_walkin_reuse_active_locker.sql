-- Multi-Session Locker Auto-Inheritance — Reuse Active Locker Occupancy Row (ohm#preventduplicateslotandsynclocker)
--
-- If a client (or active walk-in guest) already has an open locker occupancy row
-- (checked_out_at IS NULL), update that row instead of inserting a new one.
-- This avoids unique constraint violations on one_active_occupant_per_locker
-- and prevents duplicate locker occupancy rows for multi-session guests.
