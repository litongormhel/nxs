-- Locker Maintenance / Out of Order Status and Notes (ohm#4f7b9e2a)
--
-- Adds maintenance status and optional note to public.lockers
-- Adds staff_update RLS policy to public.lockers for authenticated staff
-- Adds validation in quick_walkin RPC to reject out-of-order lockers
...
-- Update quick_walkin to reject assigning out-of-order lockers
...
begin
  ...
  if p_locker_number is not null then
    if exists (
      select 1 from public.lockers
      where number = p_locker_number
        and (is_maintenance = true or status in ('out_of_order', 'maintenance'))
    ) then
      raise exception 'Locker % is out of order and cannot be assigned', p_locker_number;
    end if;
  end if;
  ...
  -- Reuse existing active locker occupancy row if one is open for this client / walk-in
  ...
  if v_existing_occ_id is not null then
    ...
    set locker_number = p_locker_number,
