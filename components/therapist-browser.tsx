...
export function TherapistBrowser({
  ...
  initialBreaks?: Record<string, Record<string, string[]>>;
  ...
  // Break slots state and confirmation modal state
  const [breaksByTherapist, setBreaksByTherapist] = useState<
    Record<string, Record<string, string[]>>
  >(() => initialBreaks ?? {});

  useEffect(() => {
    if (initialBreaks) {
      setBreaksByTherapist(initialBreaks);
    }
  }, [initialBreaks]);

  const getTherapistBreaks = (therapistName: string, date: string): string[] => {
    const tId = therapistIds[therapistName] || therapistName;
    return breaksByTherapist[tId]?.[date] ?? breaksByTherapist[therapistName]?.[date] ?? [];
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2.5">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-lg border border-gold bg-surface px-3.5 py-2 text-[11px] font-bold text-accent-gold transition hover:bg-gold/10"
        >
          <svg
            width="13"
            height="13"
          ...
          {cardRows.map((r) => {
            ...
            return (
              <TherapistCard
                ...
                bookings={bookings}
                breakSlots={getTherapistBreaks(t, viewDate)}
                isTop={isTop}
                ...
                onRequestBreak={(slot) => handleOpenBreakModal(t, slot)}
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full border text-[10.5px] font-bold transition-all ${
                          selected
                            ? "bg-gradient-to-br from-gold to-gold-hover text-black border-gold"
      ...
      {absentConfirmTherapist && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          ...
          >
            ...
            {absentConfirmBookings.length > 0 && (
              <div className="rounded-lg border border-gold/50 bg-surface-2 px-3.5 py-2.5 space-y-1.5">
      ...
      {breakModal && (() => {
        ...
        return (
          <div
            ...
            onClick={() => {
              if (!isSubmittingBreak) setBreakModal(null);
            }}
          >
            <div
              className="w-full max-w-md rounded-2xl border border-border bg-surface p-5 shadow-2xl space-y-4 animate-fade-in"
              onClick={(e) => e.stopPropagation()}
            >
              <div>
                <div className="flex items-center gap-2">
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  className="flex-1 rounded-lg border border-border py-2 text-xs font-bold text-muted hover:text-foreground transition-colors cursor-pointer"
                >
                  Cancel
                </button>
      ...
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 rounded-xl border border-gold bg-surface-2 px-5 py-2.5 font-mono text-xs font-semibold text-accent-gold shadow-2xl animate-fade-in">
          {toastMessage}
        </div>
      )}
    </div>
