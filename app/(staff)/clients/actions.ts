...
// -------------------------------------------------------------
// Offline CSV Fallback Ingestion Action
// -------------------------------------------------------------

export type ParsedOfflineRowInput = {
  date: string;
  time: string;
...
export async function importOfflineVisits(
  rows: ParsedOfflineRowInput[]
): Promise<ImportOfflineVisitsResult> {
  const supabase = await createClient();

  // 1. Session identity verification: Supervisor or Owner
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) {
  ...
  for (const row of rows) {
    ...
    if (row.matchedClientId) {
      bkQuery = bkQuery.eq("client_id", row.matchedClientId);
    } else {
      bkQuery = bkQuery
        .is("client_id", null)
        .eq("guest_label", row.clientIdentifier || "Walk-in Guest");
    }

    const { data: existingBooking } = await bkQuery.maybeSingle();
    if (existingBooking) {
      skippedCount++;
      continue;
    }
    ...
    if (bkErr) {
      console.warn("Failed to insert booking for offline visit:", bkErr);
    }

    // 2. Insert into locker_occupancy if locker provided
    if (row.lockerNumber) {
      await db.from("locker_occupancy").insert({
        locker_number: row.lockerNumber,
        client_id: row.matchedClientId || null,
        guest_label: row.matchedClientId ? null : (row.clientIdentifier || "Walk-in Guest"),
        service_id: row.serviceId,
        room_number: isWetArea ? null : (row.roomNumber || null),
        booking_id: bookingId,
        checked_in_at: visitTimestamp,
        checked_out_at: visitTimestamp,
        checked_in_by: staffRow.id,
        checked_out_by: staffRow.id,
      });
    }
  ...
  try {
    await db.from("action_logs").insert({
      staff_id: staffRow.id,
      action: "offline_csv_imported",
      detail: `Staff ${staffRow.name} imported ${importedCount} offline visits (${pointsTotal} pts credited, ${skippedCount} skipped, ₱${salesTotal} sales).`,
    });
  } catch (logErr) {
    console.warn("Failed to write to action_logs:", logErr);
  }
