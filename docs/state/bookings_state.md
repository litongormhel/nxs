...
- **(`ohm#3d7b9a2e`, 2026-09-21) Filter Pill Source Corrected**: The `availableSlots` memo previously union'd the configured `timeSlots` prop with every `dayBookings[].start_time` from live booking data. A real booking with `start_time = '11:00'` (11:00 AM, outside spa hours — legacy data) was being promoted to a stray filter pill. Fixed by removing the `dayBookings` loop — filter pills now strictly reflect the configured time slots from `weekend_slots` (via the `timeSlots` prop passed from the page server component). No source-code hardcoding of slot times was found; no DB changes required.
- **(`ohm#8b4d1c9e`, 2026-09-21) Bookings Header & Filter Bar UI Refinements**:
  - **Subtitle (`app/(staff)/bookings/page.tsx`)**: `<p className="text-sm text-muted">Manage today's massage schedule</p>` rendered directly below `<h1>Bookings</h1>`.
  - **Date Stepper Chevrons (`components/booking-browser.tsx`)**: `‹` and `›` buttons flank the date picker input. Each button applies `±1 day` via `new Date(\`${date}T00:00:00\`).setDate(...)` then calls `setDate()`, triggering the existing `useEffect([date, reloadToken])` data re-fetch. No router push; date remains local component state. Arithmetic uses local midnight to prevent UTC timezone drift.
  - **Inline Filter Bar (`components/booking-browser.tsx`)**: Search input (`w-80 max-w-xs shrink-0`) and slot pills (`All` + configured time slots) now sit side-by-side in a single `flex flex-wrap items-center gap-3` row. Record counter rendered below as a `text-right` line. Prior two-row layout (search top / pills bottom with `border-t` divider) removed. No behavior change — filtering logic, pill source, and clear-button are unchanged.
...
**Correction, `ohm#2c8f1e4a` (2026-09-21)** — Implement 100 Points Loyalty Redemption in Booking Promo Dropdown.
