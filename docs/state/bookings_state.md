...
- `components/quick-walkin-modal.tsx` — **(Rebuilt, `ohm#8r3n6y1q`) Quick
  ...
  - **(`ohm#quickwalkinsplitpay`, 2026-09-17) Split Payment UI & Backend Support**:
    `QuickWalkinModal` now includes a `Split Payment` checkbox toggle with smart auto-balancing between Method 1 and Method 2. Supports splitting payments across Cash, GCash, Card, and Maya with numeric font-mono inputs and equality validation badge (`Method 1 + Method 2 === Total Amount`). Server action `quickWalkin` (`app/(staff)/bookings/actions.ts`) accepts split payment parameters and inserts 2 separate `sales` records sharing `booking_id`, ensuring shift remittance correctly tallies both payment channels.
