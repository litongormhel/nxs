...
- **Universal Actions & Manager PIN Security Guard (`sales-browser.tsx`, `actions.ts`, `ohm#salesvoidrestorepin`, 2026-09-17)**:
  ...
  - **3-Day Lapse Rule — Owner-Only for Old Sales (`ohm#4a9e1d2c`, 2026-09-21)**: Sales older than 72 hours (3 × 24 h, wall-clock, using `sale.created_at`) are "lapsed." For non-Owner staff (Supervisor, Front Desk), the `Edit` and `Void` action buttons become `disabled` with tooltips `"Editing locked after 3 days. Owner only"` / `"Void locked after 3 days. Owner only"`. Owner always retains full access. `Restore` is unaffected. Server-side: `guardLapsedSale()` in `actions.ts` enforces the same rule for `editSale` and `voidSale` — fetches `created_at` via `adminClient`, checks age, then resolves caller `position` via `auth.getUser()` → `staff` table; non-Owners receive `{ ok: false, error: "Sales older than 3 days can only be modified or voided by the Owner." }`. No DB changes.
