...
- **Human-readable detail formatting & clean action labels** (`ohm#i35wdbgr` 2026-09-02, updated in `ohm#actloghuman` 2026-09-17) —
  ...
  human sentence per `action`. Raw snake_case `action` strings are mapped
  via `formatActionLabel(action)` to clean Title Case labels and rendered
  inside subtle gold badges in `components/logs-browser.tsx`.
  `parseDetail()` splits on **key boundaries** (`/(?:^|\s)([a-z_]+)=/g`), not spaces.
  ...
  `therapistIds` collection extracts therapist UUIDs for all therapist-related
  actions (`therapist_mark_on_leave`, `therapist_archive`, `therapist_unarchive`, etc.)
  to resolve therapist names via server-side lookup.
  Formatters strip raw technical ID dumps (`sale_id=...`, `booking_id=...`, `occupancy_id=...`)
  for user-facing entries (`quick_walkin`, `edit_booking`, `locker_checkout`, etc.),
  presenting concise, crisp business details (`text-foreground text-[12px]`).
