...
- **Two-Tab Client Profile Browser (`ohm#clienttabsandpurge`, 2026-09-17)**:
  - Top-level tab switcher in `components/client-browser.tsx`:
    - **Members**: Displays registered client accounts from `clients` table. Search filters by `@username`, codename, or member code (`#M-...`). Retains points balance badge, QR profile card modal, Log Availed Service, and immutable transaction ledger (`point_transactions`).
